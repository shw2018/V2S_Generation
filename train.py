...
if __name__ == '__main__':
    ...
    if split is not None:
        aud = glob.glob(os.path.join(args.data_dir, split[0]) + '*.tfrecords')
        vid = glob.glob(os.path.join(args.data_dir, split[1]) + '*.tfrecords')
        aud.sort()
        vid.sort()
        print(aud[0])
        print(vid[0])

        aud_test = glob.glob(os.path.join(args.data_dir, 'test_', split[0]) + '*.tfrecords')
        vid_test = glob.glob(os.path.join(args.data_dir, 'test_', split[1]) + '*.tfrecords')
        aud_test.sort()
        vid_test.sort()
